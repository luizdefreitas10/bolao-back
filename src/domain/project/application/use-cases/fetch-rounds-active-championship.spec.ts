...
import { FetchRoundsActiveChampionshipUseCase } from './fetch-rounds-active-championship'
...
let sut: FetchRoundsActiveChampionshipUseCase
...
describe('Fetch Rounds Championship', () => {
  beforeEach(() => {
    ...
    sut = new FetchRoundsActiveChampionshipUseCase(inMemoryRoundRepository)
  })
  ...
  it('should be able to fetch rounds', async () => {
    ...
    const roundInative = makeRound({
      championshipId: champ.id,
      name: ' Round Teste',
      status: 'INACTIVE',
    })
    await inMemoryRoundRepository.create(round)
    await inMemoryRoundRepository.create(roundInative)

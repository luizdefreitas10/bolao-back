...
describe('Fetch Matches by Status  (E2E)', () => {
  ...
  test('[POST] /match/by-status', async () => {
    ...
    expect(response.body).toEqual({
      matches: expect.arrayContaining([
        expect.objectContaining({
          teamIdHome: expect.objectContaining({
            value: expect.any(String),
          }),
          teamIdAway: expect.objectContaining({
            value: expect.any(String),
          }),
          status: 'WAITING',
